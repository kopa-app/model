...
describe('Schema', function () {
  ...
  it('should create a Model constructor', function () {
    ...
    expect(Schema.get()).to.have.property('User');
    expect(Schema.get().User).to.be(User);
  });
  ...
  it('should normalize fields', function () {
    ...
    Object.keys(fields).forEach(function(name) {
      ...
      expect(opts).to.have.property('serialize');
      expect(opts).to.have.property('unserialize');
    });
  });
  ...
  it('should use serializers', function () {
    ...
    expect(event.date).to.eql(date.toString());
  });
  ...
  it('should allow load and count methods on Model Constructor', function (next) {
    ...
    var Schema = Model({
      load: function(name, query, cb) {
        expect(name).to.be('User');
        expect(query).to.be(q);
        called++;
      },
      count: function(name, query, cb) {
        expect(name).to.be('User');
        expect(query).to.be(q);
        called++;
  ...
  it('should detect promises in load and count methods', function (next) {
    var called = 0;
